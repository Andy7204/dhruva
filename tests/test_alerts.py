...
class FakeGitHub:
    repo='owner/repo'
    def __init__(self): self.issues=[]; self.posts=0
    def request(self,method,path,body=None):
        if method=='GET': return [i for i in self.issues if i.get('state')!='closed']
        if method=='POST':
            self.posts+=1
            issue=dict(body,number=self.posts,html_url=f'https://github.com/owner/repo/issues/{self.posts}')
