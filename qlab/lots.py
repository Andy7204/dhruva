...
def summarize(holding):
    holding['qty']=sum(l['qty'] for l in holding['lots'])
...
def sell(holding, qty, gross, charges, exited, symbol, order_id):
    if not holding.get('lots') or sum(l['qty'] for l in holding['lots'])!=holding['qty']:
    ...
    for lot in holding['lots']:
        ...
        lot['qty']-=used;lot['economic_cost']=round(lot['economic_cost']-economic,2)
